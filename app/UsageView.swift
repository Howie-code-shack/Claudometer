...
struct UsageView: View {
    ...
    // Capped below the smallest modern Mac's below-menu-bar height so the popover
    // never overflows the screen; taller content scrolls instead.
    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            .onPreferenceChange(ContentHeightKey.self) { value in
                ...
                // Clamp here (not just in the frame) and force the popover window
                // to match. Otherwise the window follows the full content height,
                // overflows the screen, and gets shoved under the menu bar with
                // the top unreachable.
                let clamped = min(max(value, 100), maxPopupHeight)
                measuredHeight = clamped
                (NSApplication.shared.delegate as? AppDelegate)?.setPopoverHeight(clamped)
            }
